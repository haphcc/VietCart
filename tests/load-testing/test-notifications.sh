...
if [ -z "${JWT_TOKEN:-}" ]; then
  echo "Bo qua notification user endpoint vi JWT_TOKEN rong."
  echo "Chay vi du: JWT_TOKEN=your_token_here ./test-notifications.sh"
  exit 0
fi
...
run_ab() {
  ...
  "$AB_CMD" -n "$total" -c "$concurrency" -H "Authorization: Bearer $JWT_TOKEN" "$url" >> "$OUTPUT_FILE" 2>&1
  echo "" >> "$OUTPUT_FILE"
}

run_ab "Notifications /api/notifications/user/1 - tai nhe" 100 10 "$BASE_URL/api/notifications/user/1"
run_ab "Notifications /api/notifications/user/1 - tai trung binh" 500 50 "$BASE_URL/api/notifications/user/1"
run_ab "Notifications /api/notifications/user/1 - tai nang" 1000 100 "$BASE_URL/api/notifications/user/1"

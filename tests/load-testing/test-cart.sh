...
for endpoint in "/api/cart/1"; do
  run_get "Cart $endpoint - tai nhe" 100 10 "$BASE_URL$endpoint"
  run_get "Cart $endpoint - tai trung binh" 500 50 "$BASE_URL$endpoint"
  run_get "Cart $endpoint - tai nang" 1000 100 "$BASE_URL$endpoint"
done

if [ "${RUN_POST_TESTS:-0}" = "1" ]; then
  run_post "Cart POST /api/cart/items - tai nhe" 100 10 "$PAYLOAD_DIR/cart-item.json" "$BASE_URL/api/cart/items"
  run_post "Cart POST /api/cart/items - tai trung binh" 500 50 "$PAYLOAD_DIR/cart-item.json" "$BASE_URL/api/cart/items"
  run_post "Cart POST /api/cart/items - tai nang" 1000 100 "$PAYLOAD_DIR/cart-item.json" "$BASE_URL/api/cart/items"
else
  echo "Bo qua POST /api/cart/items. Dat RUN_POST_TESTS=1 neu muon chay voi payloads/cart-item.json."
fi
